...
const UIKeyMapping := preload("ui_key_mapping.tscn")

@export var is_foldable := true: set = set_is_foldable
@export var title := "": set = set_title
@export var controls := [] # (Array, String)

@onready var _controls_table : BoxContainer = find_child("ControlsTable")
@onready var _toggle_button : Button = find_child("ToggleButton")
@onready var _toggle_button_container : MarginContainer = $MarginContainer
@onready var _controls_panel : Control = find_child("ControlsPanel")
@onready var _title_label : Label = find_child("TitleLabel")


func _ready() -> void:
	_toggle_button.toggled.connect(toggle_panel)
